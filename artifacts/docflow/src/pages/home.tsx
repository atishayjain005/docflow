...
function DocumentRow({ document }: { document: { id: string; title: string; ownerId: string; ownerName: string; updatedAt: string; wordCount: number; access: string; preview: string } }) {
  ...
  return (
    <Link href={`/document/${document.id}`} data-testid={`link-document-${document.id}`} className="group flex items-center gap-3 border-b border-[hsl(var(--border)/.72)] px-5 py-4 transition hover:bg-[hsl(var(--accent)/.08)] active:bg-[hsl(var(--accent)/.14)] sm:gap-4">
...
export default function Home() {
  ...
  const documentsQuery = useListDocuments({
    request,
    query: { queryKey: [...getListDocumentsQueryKey(), currentUser.id] },
  });
  const dashboardQuery = useGetDashboard({
    request,
    query: { queryKey: [...getGetDashboardQueryKey(), currentUser.id] },
  });
  ...
  return (
    <div className="mx-auto max-w-[1440px] px-5 pb-12 pt-6 sm:px-8 md:px-10 md:pt-10 lg:px-14">
      ...
      {modal === 'import' && <Modal testId="dialog-import-document" title="Bring in a file" onClose={closeModal}><form onSubmit={handleImport}><button type="button" data-testid="button-select-import-file" onClick={() => fileInputRef.current?.click()} className="flex w-full flex-col items-center justify-center rounded-xl border border-dashed border-[hsl(var(--primary)/.45)] bg-[hsl(var(--primary)/.05)] px-5 py-10 text-center transition hover:bg-[hsl(var(--primary)/.1)] active:scale-[0.99]"><FileDown className="mb-3 h-6 w-6 text-[hsl(var(--primary))]" /><span className="text-sm font-bold">{importFile ? importFile.name : 'Choose a text or Markdown file'}</span><span className="mt-1 text-[11px] text-[hsl(var(--muted-foreground))]">We’ll keep the words, and give them a home.</span></button><input ref={fileInputRef} data-testid="input-import-file" type="file" accept=".txt,.md,.markdown,text/plain,text/markdown" className="hidden" onChange={(event) => setImportFile(event.target.files?.[0] ?? null)} /><label className="mt-5 block text-xs font-bold">Display name <span className="font-normal text-[hsl(var(--muted-foreground))]">(optional)</span><input data-testid="input-import-name" value={importName} onChange={(event) => setImportName(event.target.value)} placeholder={importFile?.name || 'Keep original filename'} className="mt-2 w-full rounded-xl border border-[hsl(var(--input))] bg-[hsl(var(--background))] px-3.5 py-3 text-sm outline-none transition focus:border-[hsl(var(--primary))] focus:ring-2 focus:ring-[hsl(var(--primary)/.12)]" /></label><button type="submit" data-testid="button-submit-import-document" disabled={!importFile || importDocument.isPending} className="mt-6 flex w-full items-center justify-center gap-2 rounded-xl bg-[hsl(var(--primary))] py-3 text-sm font-bold text-[hsl(var(--primary-foreground))] transition hover:brightness-105 active:scale-[0.99] disabled:cursor-not-allowed disabled:opacity-45">{importDocument.isPending && <LoaderCircle className="docflow-spinner h-4 w-4" />}Import document</button>{importDocument.isError && <p data-testid="status-import-error" className="mt-3 text-center text-xs text-[hsl(var(--destructive))]">That file could not be imported. Please try again.</p>}</form></Modal>}
    </div>
  );
}